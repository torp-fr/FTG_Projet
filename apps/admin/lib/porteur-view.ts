...
/**
 * Vue « voir-comme le porteur » (read-only) pour l'impersonation (Étape 2 JC-07).
 *
 * Reproduit le dashboard porteur dans SON contexte à partir des vrais états — porté depuis
 * apps/porteur/lib/data.ts (getPorteurDashboard), en lecture pure. L'opérateur voit ce que le
 * porteur voit ; il n'agit sur rien (aucune écriture métier). La trace d'impersonation est
 * écrite séparément par la page (audit non contournable).
 */
...
async function idMap(c: Client, table: "segments" | "gates" | "milestones", col: "name" | "code", ids: string[]): Promise<Map<string, string>> {
...
export interface JalonNode { code: string; state: string; quality: number | null; recommended: boolean; reason?: string }
...
export interface DeliverableView { id: string; type: string; title: string; version: number; status: string; createdAt: string; data: Record<string, unknown> }
export interface AgentView { persona: string; code: string; latestRunStatus: string | null; runsCount: number; outputs: Array<{ label: string; deliverableId: string | null }> }

export interface PorteurView {
...
export async function getPorteurView(projectId: string): Promise<PorteurView | null> {
  ...
  const { data: pRaw } = await c
    ...
    .maybeSingle();
  const p = pRaw as {
    id: string; name: string; entry_door: string; ambition_profile: string | null; status: string; owner_user_id: string; segment_primary_id: string | null;
  } | null;
  if (!p) return null;
  ...
  const agents: AgentView[] = engines.map((e) => {
    ...
    const outputs: AgentView["outputs"] = dels
      .filter((d) => d.engine_run_id && myRunIds.has(d.engine_run_id))
      .map((d) => ({ label: d.title, deliverableId: d.id as string | null }));
    if (e.code === "founder_profiler" && mantra !== null) outputs.unshift({ label: "Profil d’incarnation", deliverableId: null });
