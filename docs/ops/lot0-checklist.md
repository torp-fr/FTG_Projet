...
- [x] **Repo GitHub synchronisé** (`torp-fr/FTG_Projet`, public) — push automatisé toujours bloqué côté proxy de session (limite connue, sans solution trouvée depuis l'intérieur de la session), mais le contenu est aligné via upload manuel : structure complète confirmée identique fichier par fichier entre le repo local et GitHub au 11/07/2026
...
**Lot 0 non-bloquant pour la suite** : le Lot 1 (schéma + seed + orchestrateur v1) a démarré et s'est terminé le 11/07/2026 directement sur `ftg-plateforme`, sans attendre les deux derniers items ci-dessus (comptes sources et coffre) — voir `docs/ops/lot1-status.md` et le rapport §7 du plan d'exécution (Claude Project).
