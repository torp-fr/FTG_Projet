/**
 * Écritures/lectures Supabase de l'engine E7. Serveur uniquement (createFtgServiceClient).
 * E7 N'ÉCRIT PAS dans gate_evaluations. Helpers db locaux au package (pattern E1..E5).
 */
...
/** Contexte projet utile à E7 : segment + idée retenue (selection_brief). */
export async function readProjectForE7(
  client: FtgClient,
  projectId: string,
): Promise<{ segment: string | null; idea: string | null }> {
...
export interface LegalStructureInput {
  projectId: string;
...
/**
 * Insère le livrable de structure juridique (type='legal_structure', status='delivered').
 * version incrémentée par (project_id, type).
 */
export async function writeLegalStructure(client: FtgClient, d: LegalStructureInput) {
  const { data: existing } = await client
    .from("deliverables")
    .select("version")
    .eq("project_id", d.projectId)
    .eq("type", "legal_structure")
    .order("version", { ascending: false })
    .limit(1)
    .maybeSingle();
  ...
  const { data, error } = await client
    ...
    .select()
    .single();
  if (error) throw new Error(`writeLegalStructure: ${error.message}`);
  return data;
}
