/**
 * Écritures/lectures Supabase de l'engine E9. Serveur uniquement (createFtgServiceClient).
 * E9 N'ÉCRIT PAS dans gate_evaluations. Helpers db locaux au package (pattern E1..E8).
 */
...
/** Contexte projet utile à E9 : segment + idée retenue (selection_brief) + lentilles géo. */
export async function readProjectForE9(
  client: FtgClient,
  projectId: string,
): Promise<{ segment: string | null; idea: string | null; geoLenses: string[] }> {
  const { data: proj } = await client.from("projects").select("segment_primary_id, geo_lenses").eq("id", projectId).maybeSingle();
...
export interface NamingReportInput {
  projectId: string;
...
/** Insère le rapport de naming (type='naming_report', status='delivered'). version par (project_id, type). */
export async function writeNamingReport(client: FtgClient, d: NamingReportInput) {
  const { data: existing } = await client
    .from("deliverables")
    .select("version")
    .eq("project_id", d.projectId)
    .eq("type", "naming_report")
    .order("version", { ascending: false })
    .limit(1)
    .maybeSingle();
  ...
  const { data, error } = await client
    ...
    .select()
    .single();
  if (error) throw new Error(`writeNamingReport: ${error.message}`);
  return data;
}
