/**
 * Écritures/lectures Supabase de l'engine E8. Serveur uniquement (createFtgServiceClient).
 * E8 N'ÉCRIT PAS dans gate_evaluations. Helpers db locaux au package (pattern E1..E7).
 */
...
/** Contexte projet utile à E8 : segment + idée retenue (selection_brief). */
export async function readProjectForE8(
  client: FtgClient,
  projectId: string,
...
export interface TaxEducationInput {
  projectId: string;
...
/** Insère le livrable fiscal (type='tax_education', status='delivered'). version par (project_id, type). */
export async function writeTaxEducation(client: FtgClient, d: TaxEducationInput) {
  const { data: existing } = await client
    .from("deliverables")
    .select("version")
    .eq("project_id", d.projectId)
    .eq("type", "tax_education")
    .order("version", { ascending: false })
    .limit(1)
    .maybeSingle();
  ...
  const { data, error } = await client
    ...
    .select()
    .single();
  if (error) throw new Error(`writeTaxEducation: ${error.message}`);
  return data;
}
